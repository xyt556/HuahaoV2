...
def Cust_Binarization():
    ...
    # 去除NaN？？？？
    camera_c = np.array(dataset.GetRasterBand(1).ReadAsArray())

    camera2 = camera_c.flatten()
    i_min = np.nanmin(camera2)
    i_max = np.nanmax(camera2)
    print(i_max, i_min)
    camera2 = [i for i in camera2 if (i >= i_min and i <= i_max)]  # i >= -1 and i <= 1
    ...
    plt.figure(figsize=(9, 4))
    plt.subplot(131)
    plt.imshow(camera_c, cmap='hot', interpolation='nearest')
    plt.axis('off')
    print(tval.get())

    val = float(tval.get())
    print('自定义阈值是：',val)
    camera_c[camera_c <= val] = 0  # np.nan
    camera_c[camera_c > val] = 1  # np.nan
    # im = exposure.rescale_intensity(camera, out_range='float')
    # im = img_as_float(im)
    ...
    print(Bin_img_cust)
    rd.write_img(Bin_img_cust, im_proj, im_geotrans, camera_c)
    ...
    plt.subplot(132)
    plt.imshow(camera2, cmap='gray', interpolation='nearest')
    plt.axis('off')
    plt.subplot(133)
...
tval = tk.Entry(root)
tval.grid(column=1,row=5)
tval.insert(0, "0")
b3 = tk.Button(root, text = '二值化',font=("宋体",10, "bold"),width = 8, height =2, command = Binarization)
b3.grid(column=2,row=5)
b4 = tk.Button(root, text = '自定义二值化',font=("宋体",10, "bold"),width = 16, height =2,command = Cust_Binarization)
